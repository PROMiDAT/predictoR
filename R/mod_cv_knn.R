  ...
  tagList(
    tabBoxPrmdt(
      ...
      tabPanel(title = p(labelInput("indices"),class = "wrapper-tag"), value = "tabCVKnnIndices",
               div(col_6(echarts4rOutput(ns("e_knn_glob"), width = "100%")),
                   col_6(echarts4rOutput(ns("e_knn_error"), width = "100%")))),
      tabPanel(title = p(labelInput("indicesCat"),class = "wrapper-tag"), value = "tabCVKnnIndicesCat",
               div(col_12(echarts4rOutput(ns("e_knn_category"), width = "100%"))))
    )
...
mod_cv_knn_server <- function(input, output, session, updateData, codedioma){
    ...
    output$e_knn_glob  <-  renderEcharts4r({
      ...
      if(!is.null(grafico)){
        ...
        resumen.puntos(grafico, labels = c(tr("precG",idioma), tr("iter",idioma)))
      }
      else
        return(NULL)
    })    
    ...
    output$e_knn_error  <-  renderEcharts4r({
      ...
      if(!is.null(M$grafico)){
        ...
        resumen.puntos(err, labels = c(tr("errG",idioma), tr("iter",idioma)))
    ...
    output$e_knn_glob3  <-  renderEcharts4r({
      input$btn_cv_knn
      prec    <- input$cvknn.glo
      idioma  <- codedioma$idioma
      grafico <- M$grafico
      if(!is.null(grafico)){
        if(prec == "1"){
          ...
          resumen.puntos(grafico, labels = c(tr("errG",idioma), tr("iter",idioma)))
        }
      }
      else
        return(NULL)
    ...
    output$e_knn_category  <-  renderEcharts4r({
      ...
      if(!is.null(M$grafico)){
        ...
        resumen.puntos(graf, labels = c(paste0(tr("prec",idioma), " ",cat ), tr("iter",idioma)))
