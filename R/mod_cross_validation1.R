...
mod_cross_validation11_ui <- function(id){
  ns <- NS(id)
  title_comp <- list(conditionalPanel("input['cross_validation1_ui_1-BoxCV'] == 'tabcvcvIndicesCat'",
                                      div(id = ns("row"), shiny::h5(style = "float:left;margin-top: 15px;margin-right: 10px;", labelInput("selectCat"),class = "wrapper-tag"),
                                          tags$div(class="multiple-select-var",
                                                   selectInput(inputId = ns("cv.cat.sel"),label = NULL,
                                                               choices =  "", width = "100%")))),
                     conditionalPanel("input['cross_validation1_ui_1-BoxCV'] == 'tabcvcvIndices3'",
...
#' cross_validation1 Server Functions
#'
#' @noRd 
mod_cross_validation1_server <- function(input, output, session, updateData, codedioma){
    ns <- session$ns
    ...
    observeEvent(input$btn_cv, {
      output$txt_cv <- renderPrint({
        tryCatch({
          ...
          MCs.lda          <- vector(mode = "list", length = cant.vc)
          MCs.qda          <- vector(mode = "list", length = cant.vc)
          ...
          for(i in 1:cant.vc){
            ...
            for(k in 1:numGrupos) {
              ...
              modelo     <- train.lda(var_, data = ttraining)
              prediccion <- predict(modelo, ttesting)
              MC         <- confusion.matrix(ttesting, prediccion)
              MC.lda     <- MC.lda + MC
              
              modelo     <- train.qda(var_, data = ttraining)
              prediccion <- predict(modelo, ttesting)
              MC         <- confusion.matrix(ttesting, prediccion)
              MC.qda     <- MC.qda + MC
          ...
          MCs.cv <- list(MCs.svm          = MCs.svm,          MCs.knn        = MCs.knn, 
                         ...
                         MCs.regrLogP     = MCs.regrLogP,     MCs.lda        = MCs.lda,
                         MCs.qda          = MCs.qda)
          
          methods <- c("knn", "arboles", "potenciacion",  "bosques", "svm", 
                       "bayes", "xgboosting", "regrLog", "regrLogP", "lda", "qda")
          M$MCs.cv     <- MCs.cv
          resultados   <- indices.cv(category, cant.vc, methods, MCs.cv)
          resultados$grafico$name <- factor(tr(c("knnl", "dtl", "rfl", "bl", "svml", "Bayes", "xgb" , "rl", "rlr", "lda", "qda"),codedioma$idioma))
    ...
    output$e_cv_precision  <-  renderEcharts4r({
      ...
      tryCatch({
        ...
        if(!is.null(grafico)){
          ...
          switch (type,
                  "barras" = return( resumen.barras(grafico, labels = c(tr("errG",idioma), tr("modelo", idioma) ))), 
                  "error" = return( resumen.error(grafico, labels = c(tr("errG",idioma), tr("modelo", idioma), tr("maximo", idioma),tr("minimo", idioma)))), 
                  "lineas" = return( resumen.lineas(grafico, labels = c(tr("errG",idioma), tr("crossval",idioma) )))
          )
        }
        else
          return(NULL)
...
# mod_cross_validation1_ui("cross_validation1_1")
    
## To be copied in the server
# mod_cross_validation1_server("cross_validation1_1")
