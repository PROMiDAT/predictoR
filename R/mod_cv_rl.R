  ...
  tagList(
    tabBoxPrmdt(
      ...
      tabPanel(title = p(labelInput("seleParModel"),class = "wrapper-tag"), value = "tabCVrlModelo",
               fluidRow(col_6(numericInput(ns("cvrl_step"), labelInput("probC"), value = 0.5, width = "100%", min = 0, max = 1)),
      ...
      tabPanel(title = p(labelInput("indicesCat"),class = "wrapper-tag"), value = "tabCVrlIndicesCat",
               div(col_12(echarts4rOutput(ns("e_rl_category"), width = "100%", height = "70vh"))))
    )
...
mod_cv_rl_server <- function(input, output, session, updateData, codedioma){
    ...
    observeEvent(c(updateData$datos, updateData$variable.predecir), {
      ...
      if(!is.null(datos)){
        ...
        updateSelectInput(session, "cvrl.sel", choices = choices, selected = choices[1])
      ...
      tryCatch({
        ...
        for (i in 1:cant.vc){
          ...
          for (k in 1:numGrupos){
            ...
            j <- 1
              modelo      <- train.glm(as.formula(var_), data = ttraining)
