...
mod_cv_qda_ui <- function(id){
  ...
  title_comp <- list(conditionalPanel("input['cv_qda_ui_1-Boxqda'] == 'tabCVqdaIndicesCat'",
                                      div(id = ns("row"), shiny::h5(style = "float:left;margin-top: 15px;margin-right: 10px;", labelInput("selectCat"),class = "wrapper-tag"),
                                          tags$div(class="multiple-select-var",
                                                   selectInput(inputId = ns("cvqda.sel"),label = NULL,
                                                               choices =  "", width = "100%")))))
...
mod_cv_qda_server <- function(input, output, session, updateData, codedioma){
    ...
    observeEvent(c(updateData$datos, updateData$variable.predecir), {
      ...
      if(!is.null(datos)){
        ...
        updateSelectInput(session, "cvqda.sel", choices = choices, selected = choices[1])
    ...
    output$txtcvqda <- renderPrint({
      ...
      tryCatch({
        ...
        nombre    <- "MC.qda"
        Corte     <- isolate(input$cvqda_step)
        cat_sel   <- isolate(input$cvqda_cat)
